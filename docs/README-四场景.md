...
|---|---|---|---|
| server(公司 Linux) | `CHANGE_ME_SERVER_TAILNET_IP` | systemd user `remote-terminal` ✅ | `CHANGE_ME_TOKEN` |
| 家 Mac(my-macbook) | `CHANGE_ME_MAC_TAILNET_IP` | launchd `com.calla.remote-terminal` ✅(原方案已装) | 见 Mac 上 `~/remote-terminal/server/config.json` |
...
App 填:主机 `CHANGE_ME_SERVER_TAILNET_IP` / 端口 `7681` / token `CHANGE_ME_TOKEN`。

## 场景 2:手机 → 家 Mac(原方案,已具备)

App 填:主机 `CHANGE_ME_MAC_TAILNET_IP` / 端口 `7681` / token = Mac 的 config.json 里那个。
...
RT_TOKEN=<Mac的token> node ~/remote-terminal/client/rt-client.js CHANGE_ME_MAC_TAILNET_IP -a shell
# 或直接进 Mac 的 tmux 持久会话:
RT_TOKEN=<Mac的token> node ~/remote-terminal/client/rt-client.js CHANGE_ME_MAC_TAILNET_IP -a tmux -s mac-work
```
...
npm install ws
# 之后每次:
RT_TOKEN=CHANGE_ME_TOKEN node rt-client.js CHANGE_ME_SERVER_TAILNET_IP -a shell
```
